...
| MCP | Replace the `unreadOnly` and `starredOnly` arguments with `read` and `starred`. Treat `list_emails.text` as literal text and use typed syntax for `search_emails.query`. |
