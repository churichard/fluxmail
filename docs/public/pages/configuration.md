...
For a personal Gmail setup, `fluxmail config set` is the simplest: set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` once and every command and server mode finds them. IMAP accounts do not need those settings. Use `fluxmail config list` to review stored settings (secret values are masked) and `fluxmail config unset <KEY>` to remove one.
...
| `FLUXMAIL_PUBLIC_URL` | `http://localhost:<FLUXMAIL_PORT>` | Public base URL used for the MCP endpoint and hosted Gmail callback. |
| `FLUXMAIL_AUTH` | `apikey` | HTTP authentication mode. Use none only behind a trusted network boundary. |
| `FLUXMAIL_OAUTH_PORT` | `8976` | Port for the local Gmail OAuth callback listener. |
| `FLUXMAIL_OAUTH_HOST` | `127.0.0.1` | Bind address for the local Gmail OAuth callback listener. |
