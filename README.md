...
3. Create credentials → **OAuth client ID** → type **Web application**, with these authorized redirect URIs:
   ...
   - `<your FLUXMAIL_PUBLIC_URL>/auth/google/callback` (only if you use the server-hosted flow on a remote deployment)
...
**Option B: Docker (Streamable HTTP server).** For a shared or remote deployment. The image is published to GitHub Container Registry as [`ghcr.io/churichard/fluxmail-mcp`](https://github.com/churichard/fluxmail-mcp/pkgs/container/fluxmail-mcp) for amd64 and arm64.
